...
export class RacingView {
 ...
 startRanked(){this.game.reset();this.paddle=0;this.running=true;this.updateStatus();this.draw();}
 ...
 constructor(plan:FloorPlan,f:Furniture,private scene:THREE.Scene){
  ...
  const move=(e:PointerEvent)=>{if((e.target as HTMLElement).closest('button')||this.phase!=='game'||(e.pointerType==='mouse'&&!e.buttons))return;const a=this.screenPoint(-.5,0),b=this.screenPoint(.5,0);this.paddle=THREE.MathUtils.clamp(((e.clientX-a.x)/(b.x-a.x)-.5)*2,-1,1);};
  ...
  this.ui.addEventListener('pointerup',()=>this.paddle=0);this.ui.addEventListener('pointercancel',()=>this.paddle=0);
 ...
 key(code:string,down:boolean){
  if(this.active){if(['KeyA','KeyD','ArrowLeft','ArrowRight'].includes(code)){if(down)this.keys.add(code);else this.keys.delete(code);return true;}}
 ...
 enter(){
  ...
  this.phase='in';this.time=0;this.focus=0;this.running=false;this.keys.clear();this.ui.hidden=false;this.hidePrompt();document.body.classList.add('racing-playing');this.updateStatus();this.onEnter();
 }
 ...
 clearInput(){this.keys.clear();this.paddle=0;}
 ...
 update(camera:THREE.PerspectiveCamera,dt:number){
  ...
  if(this.phase==='game'&&this.running){
   ...
   this.game.step(dt,keyboard||this.paddle);this.updateStatus();
