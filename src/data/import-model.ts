...
export function parseHouseModel(text:string):Record<FloorId,FloorPlan> {
  ...
  for(const id of floorOrder) {
    ...
    const plan:FloorPlan={
      ...
      furniture:list(f.furniture,100,v=>{const item=object(v);return {kind:oneOf<Furniture['kind']>(item.kind,furnitureKinds),rect:rect(item.rect),height:number(item.height,['rug','kitchenSplash','ceilingSpot','diningCove','livingSpeaker'].includes(item.kind as string)?.005:.15,5),...(item.bottom===undefined?{}:{bottom:number(item.bottom,0,5)}),...(item.workstation===undefined?{}:{workstation:oneOf(item.workstation,['wide','standard'] as const)}),...(item.finish===undefined?{}:{finish:oneOf(item.finish,['dining','living'] as const)}),...(item.kitchen===undefined?{}:{kitchen:oneOf(item.kitchen,kitchenKinds)}),facing:item.facing===undefined?undefined:oneOf<Furniture['facing'] & string>(item.facing,['north','south','east','west'])};}),
