-- Add the nine-dart challenge without changing existing scores or access grants.
begin;
alter table public.game_runs drop constraint game_runs_game_check;
alter table public.game_runs add constraint game_runs_game_check check (game in ('pong','boxing','tic-tac-toe','darts'));
alter table public.game_scores drop constraint game_scores_game_check;
alter table public.game_scores add constraint game_scores_game_check check (game in ('pong','boxing','tic-tac-toe','darts'));
...
begin
  ...
  if p_game not in ('pong','boxing','tic-tac-toe','darts') then raise exception 'INVALID_GAME'; end if;
